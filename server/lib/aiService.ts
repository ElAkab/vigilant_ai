...
export class AIService {
  ...
  async generateContent(prompt: string): Promise<AIResponse> {
    ...
    throw new HttpError(502, 'AI_SERVICE_ERROR', `All OpenRouter models failed: ${lastError?.message}`)
  }
  ...
  private async callOpenRouter(model: AIModel, prompt: string): Promise<AIResponse> {
    const apiKey = getEnv('OPENROUTER_API_KEY')
    if (!apiKey) {
  ...
  private async generateMockContentStream(
    ...
    async function* gen() {
      const chunks = [`MOCK: start`, `MOCK: mid`, `MOCK: end for prompt ${prompt.split('\n')[0]}`]
